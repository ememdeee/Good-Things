...
if ( ! function_exists( 'understrap_post_nav' ) ) {
	...
	function understrap_post_nav() {
		...
		<nav class="container navigation post-navigation">
			<h2 class="visually-hidden"><?php esc_html_e( 'Post navigation', 'understrap' ); ?></h2>
...
// Add a new column to the case-study post type
function cd_add_case_study_columns($columns) {
    $columns['case_study_client'] = __('Client', 'textdomain');
    return $columns;
}
add_filter('manage_case-study_posts_columns', 'cd_add_case_study_columns');

// Populate the new column with the case_study_client field value
function populate_case_study_columns($column, $post_id) {
    if ('case_study_client' === $column) {
        $case_study_client = get_field('case_study_client', $post_id);
        if ($case_study_client) {
            echo esc_html($case_study_client);
        } else {
            echo __('N/A', 'textdomain');
        }
    }
}
add_action('manage_case-study_posts_custom_column', 'populate_case_study_columns', 10, 2);