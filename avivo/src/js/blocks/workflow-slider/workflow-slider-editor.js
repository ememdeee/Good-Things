 ...
 *   or modified.
 * - `workflowDebounceTimer`: A variable to store the debounce timer, which prevents rapid re-initialization of the slider.
 * - `DEBOUNCE_DELAY`: The delay time (in milliseconds) before the slider re-initializes after a change is detected.
 * - The slider initialization is managed through the `initializeWorkflowSlider()` function, which is imported from 
 *   "workflow-slider-common.js".
 *
 * Usage:
...
wp.domReady(() => {
	if (typeof wp.data !== 'undefined') {
		let workflowDebounceTimer;		// Variable to store the debounce timer for the slider
		...
		wp.data.subscribe(() => {
			...
			if (JSON.stringify(previousContent) !== JSON.stringify(currentContent)) {
				clearTimeout(workflowDebounceTimer);
				workflowDebounceTimer = setTimeout(() => {
					initializeWorkflowSlider();
					//console.log('workflow-slider re-initialized');
