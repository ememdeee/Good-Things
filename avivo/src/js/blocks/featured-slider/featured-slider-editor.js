 ...
 *
 * The `@prepros-prepend` directive is used by Prepros to include the shared functions from "featured-slider-common.js"
 * at the top of this file. This ensures that the `initializeFeaturedSlider()` function is available for use within this script.
 *
 * Key elements:
 ...
 *   or modified.
 * - `featuredsliderDebounceTimer`: A variable to store the debounce timer, which prevents rapid re-initialization of the slider.
 * - `DEBOUNCE_DELAY`: The delay time (in milliseconds) before the slider re-initializes after a change is detected.
 * - The slider initialization is managed through the `initializeFeaturedSlider()` function, which is imported from 
 *   "featured-slider-common.js".
 *
 * Usage:
...
// @prepros-prepend "featured-slider-common.js";

wp.domReady(() => {
	if (typeof wp.data !== 'undefined') {
		let featuredsliderDebounceTimer;	// Variable to store the debounce timer for the slider
		...
		wp.data.subscribe(() => {
			...
			if (JSON.stringify(previousContent) !== JSON.stringify(currentContent)) {
				clearTimeout(featuredsliderDebounceTimer);
				featuredsliderDebounceTimer = setTimeout(() => {
					initializeFeaturedSlider();
					//console.log('featured-slider re-initialized');
