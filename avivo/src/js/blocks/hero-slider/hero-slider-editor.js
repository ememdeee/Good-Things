 ...
 *
 * The `@prepros-prepend` directive is used by Prepros to include the shared functions from "hero-slider-common.js"
 * at the top of this file. This ensures that the `initializeHeroSlider()` function is available for use within this script.
 *
 * Key elements:
 ...
 *   or modified.
 * - `herosliderDebounceTimer`: A variable to store the debounce timer, which prevents rapid re-initialization of the slider.
 * - `DEBOUNCE_DELAY`: The delay time (in milliseconds) before the slider re-initializes after a change is detected.
 * - The slider initialization is managed through the `initializeHeroSlider()` function, which is imported from 
 *   "hero-slider-common.js".
 *
 * Usage:
...
// @prepros-prepend "hero-slider-common.js";

wp.domReady(() => {
	if (typeof wp.data !== 'undefined') {
		let herosliderDebounceTimer;	// Variable to store the debounce timer for the slider
		...
		wp.data.subscribe(() => {
			...
			if (JSON.stringify(previousContent) !== JSON.stringify(currentContent)) {
				clearTimeout(herosliderDebounceTimer);
				herosliderDebounceTimer = setTimeout(() => {
