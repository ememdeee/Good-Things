 ...
 *
 * The `@prepros-prepend` directive is used by Prepros to include the shared functions from "testimonial-slider-small-common.js"
 * at the top of this file. This ensures that the `initializeTestimonialSliderSM()` function is available for use within this script.
 *
 * Key elements:
 ...
 *   or modified.
 * - `testimonialSmDebounceTimer`: A variable to store the debounce timer, which prevents rapid re-initialization of the slider.
 * - `DEBOUNCE_DELAY`: The delay time (in milliseconds) before the slider re-initializes after a change is detected.
 * - The slider initialization is managed through the `initializeTestimonialSliderSM()` function, which is imported from 
 *   "testimonial-slider-small-common.js".
 *
 * Usage:
...
// @prepros-prepend "testimonial-slider-small-common.js";

wp.domReady(() => {
	if (typeof wp.data !== 'undefined') {
		let testimonialSmDebounceTimer;	// Variable to store the debounce timer for the slider
		...
		wp.data.subscribe(() => {
			...
			if (JSON.stringify(previousContent) !== JSON.stringify(currentContent)) {
				clearTimeout(testimonialSmDebounceTimer);
				testimonialSmDebounceTimer = setTimeout(() => {
					initializeTestimonialSliderSM();
