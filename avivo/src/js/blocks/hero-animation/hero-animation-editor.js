 ...
 *
 * The `@prepros-prepend` directive is used by Prepros to include the shared functions from "hero-animation-common.js"
 * at the top of this file. This ensures that the `initializeHeroAnimation()` function is available for use within this script.
 *
 * Key elements:
 ...
 *   or modified.
 * - `heroAnimationDebounceTimer`: A variable to store the debounce timer, which prevents rapid re-initialization of the animation.
 * - `DEBOUNCE_DELAY`: The delay time (in milliseconds) before the animation re-initializes after a change is detected.
 * - The animation initialization is managed through the `initializeHeroAnimation()` function, which is imported from
 *   "hero-animation-common.js".
 *
 * Usage:
 * - This script is intended for use within the block editor. It keeps the animation functionality consistent and
...
wp.domReady(() => {
	if (typeof wp.data !== 'undefined') {
		let heroAnimationDebounceTimer;	// Variable to store the debounce timer for the animation
		...
		wp.data.subscribe(() => {
			...
			if (JSON.stringify(previousContent) !== JSON.stringify(currentContent)) {
				clearTimeout(heroAnimationDebounceTimer);
				heroAnimationDebounceTimer = setTimeout(() => {
					initializeHeroAnimation();
					//console.log('hero-animation re-initialized');
